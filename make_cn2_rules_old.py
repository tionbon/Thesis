...
def find_and_evaluate_complement_rules(rule, dataset, scores, tag, k):
	...
	for col in obscured_and_original_cols:
		...
		for group_value in group_values[split_col]:
			specific_covered_data = covered_data.query(group_value[0])
			unique_vals = specific_covered_data[col].unique() if col not in original else [group_value[2]]
			op = group_value[1]
	...
	for comb in all_combinations:
		...
		# filter out rules that have been counted already, and do not have same form as original
